...
wrong, or which problems it is the wrong tool for. This chapter is the mechanics.
...
- **Supervised learning**: a model learns a mapping from labeled examples.
- **Neural network**: layers of weighted connections tuned by training.
- **Large language model**: a network trained to predict the next token over enormous text.
- **Transformer and attention**: the architecture that lets a model weigh distant context.
- **Token**: the unit a model reads and writes, roughly a word piece. Cost and limits are counted in tokens.
- **Context window**: how much the model can attend to at once.
